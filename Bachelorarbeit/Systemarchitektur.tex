...
In diesem Kapitel wird die Systemarchitektur der Anwendung vorgestellt, indem erläutert wird wie die verschiedenen Komponenten und Technologien zusammenarbeiten und miteinander kommunizieren. Des weiteren werden verschiedene Abläufe durch Aktivitäts- und Sequenzdiagramme verdeutlicht.
\section{Einführung}
...
In der zweiten Middleware tritt die Socket der \verb|userid| des Benutzers bei und wird sowohl in der Redis Datenbank unter \verb|user:username| (siehe Kapitel \ref{sec:redis-data}) , als auch bei Freunden mittels des Events \verb|connected|, als online vermerkt.
Der Beitritt der eigenen \verb|userid| dient als Kommunikationsschnittstelle. So können Sockets anderer Benutzer immer an diese \verb|userid| senden um ein Event an diesen Benutzer zu senden.
...
Implementierungsdetails, wie ein Schachspiel auf dem Server mittels der Methoden \linebreak \verb|createChessGame| und \verb|initializeGame| initialisiert wird und wie die Informationen des Schachspiels an das Frontend gesendet werden wird in Kapitel \ref{sec:Schach-Backend-impl} erläutert.
...
Die Benutzernamen werden aus dem Redis Eintrag \verb|game:roomId| geholt und falls es sich um angemeldete Benutzer handelt wird das entsprechende Element aus der \verb|activeGames| Liste von \verb|user:username| gelöscht. Anschließend wird der Eintrag \verb|game:roomId| gelöscht.
...
\item \textit{activeGames:} Eine Liste in der alle roomIds der aktuellen Spiele gespeichert sind
\end{itemize}
